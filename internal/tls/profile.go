...
// getTimeout bounds the startup read of apiservers/cluster so a hung
// Get is treated as a failed read and falls back to the platform default.
...
// the platform default and records StrictAllComponents, the adherence under
// which that default is honoured, so the watcher restarts once the real object
// shows a clean start would serve something else. A non-nil apiServer uses its
// TLSSecurityProfile and TLSAdherence. watch sets whether SetupProfileWatch
// will install a watcher for this Profile.
